...
class NNFMLoss(torch.nn.Module):
    def __init__(self, device):
        ...
        self.vgg = torchvision.models.vgg16(weights=None)  # 或 pretrained=False

        models_dir = Path(__file__).resolve().parents[1] / "models"
        state_dict_path = models_dir / "vgg16-397923af.pth"
        if not state_dict_path.exists():
            raise FileNotFoundError(f"VGG16 loss weights file not found: {state_dict_path}")
        ...
        # 3. 再 eval + to(device)
        self.vgg = self.vgg.eval().to(device)
    ...
    def forward(
        ...
        for block in blocks:
            ...
            s_feats = torch.cat([self.s_feats_all[ix_map[ix]] for ix in layers], 1)
