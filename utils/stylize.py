...
from imager_IO_utils import misc
...
def produce_stylization(content_im, style_im, phi,
    ...
    for scl in range(max_scls)[::-1]:
        print(style_pyr[scl].shape)
        print(c_pyr[scl].shape)
        print(scl)
        ...
        write_Single_Img(style_im_tmp,f'debugs/s{scl}.png')
        write_Single_Img(content_im_tmp,f'debugs/c{scl}.png')
        write_Single_Img(output_im_tmp,f'debugs/o{scl}.png')
        
        print(content_im_tmp.shape)
        print(style_im_tmp.shape)
        print(output_im_tmp.shape)
        li += 1
...
import numpy as np
def write_Single_Img(image,path):
    new_im_out = np.clip(image[0].permute(1, 2, 0).detach().cpu().numpy(), 0., 1.)
    # Save stylized output
    save_im = (new_im_out * 255).astype(np.uint8)
    imwrite(path, save_im)