...
def get_unimatch_model(weights_path=None):
    default_weights = Path(__file__).resolve().parents[1] / "models" / "gmflow-scale2-regrefine6-sintelft-6e39e2b9.pth"
    weights_path = Path(weights_path) if weights_path is not None else default_weights
    if not weights_path.exists():
        raise FileNotFoundError(f"UniMatch 权重文件缺失: {weights_path}")
...
def get_flow_simple(unimatch, image1,image2):
    bs = 1
    device = 'cuda'
    padding_factor = 32
    ...
    pred_flows = results_dict['flow_preds'][-1]
    ...
    pred_flows_back = pred_flows_back.unsqueeze(0)
    pred_occs, backward_occ = forward_backward_consistency_check(pred_flows, pred_flows_back)
    
    return pred_flows, pred_flows_back,pred_occs,backward_occ
