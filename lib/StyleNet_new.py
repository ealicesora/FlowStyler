...
class UNet(nn.Module):
    def __init__(self, ngf=16, input_channel=3, output_channel=3):
        ...
        # self.apply(reset_weights_kaiming)
    
        # self.conv_fin.weight.data.zero_()
        # self.conv_fin.bias.data.zero_()
        
        self.trend = nn.Parameter(torch.tensor(0.001, requires_grad=True))
        
    def forward(self, x_in,use_sigmoid=True,direct_output = False):
        ...
        final = h *self.trend #* 100.0
        # print(final.shape)
        return final + x_in # * (1.0-h)


def reset_weights_kaiming(m):
